	...
	*/
	...
	public:
		...
		*/
		...
		/** Output the binary code and binary mask. */
		void outputBinaryCode(const CvMat * pApplicationPoints, const char * filename);
		void releaseBinaryCode();
		char * convertToBinary(IplImage * img, int &sn);
		char * impConvert2Binary(IplImage * appPoints, IplImage * img, int &sn);
		...
		*/
		...
		float binaryMatch(OsiEye & rEye, const CvMat * pApplicationPoints);
