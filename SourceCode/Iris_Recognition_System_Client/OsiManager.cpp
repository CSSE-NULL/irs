...
namespace osiris
{
    ...
    void OsiManager::loadConfiguration(const string & rFilename)
    {
        ...
        {
            loadGaborFilters();
            loadApplicationPoints();//edited by zdd.
        }
    ...
    //这里是从list of images的配置参数路径来获取所有图片名称并保存到mListOfImages
    ...
    void OsiManager::processOneEye(char* rFileName, OsiEye & rEye)
    {
        ...
        // Get eye name
        char* fname = strrchr(rFileName, '\\');
        string short_name = osu.extractFileName(fname+1);
        ...
        if (mProcessEncoding)
        {
            ...
            rEye.outputBinaryCode(mpApplicationPoints, short_name.c_str());
        }
    ...
    OsiManager::Result OsiManager::getResult(const char* filename) {
        Result result;
        char path[100];
        sprintf(path, "code/BinaryCode/%s.txt", filename);
        ifstream file(path, ios::in);
        char line[M];
        file >> line;
        memset(result.code, 0, M);
        strcpy(result.code, line);
        file >> line;
        memset(result.mask, 0, M);
        strcpy(result.mask, line);
        file.close();
        char path2[100];
        sprintf(path2, "code/shiftedBinaryCode/%s.txt",filename);
        ifstream file2(path2, ios::in);
        for (int i = 0; i < STN; i++) {
            file2 >> line;
            strcpy(result.shift_code[i], line);
        }
        file2.close();
        if (!mTest) {
            remove(path);
